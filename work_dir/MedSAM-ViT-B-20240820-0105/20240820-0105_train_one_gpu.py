...
class NpyDataset(Dataset):
    def __init__(self, data_root, bbox_shift=20):
        ...
        self.gt_path_files = [
            file
            for file in self.gt_path_files
            if os.path.isfile(os.path.join(self.img_path, '_'.join(os.path.basename(file).split('_')[:2]) + ".npy"))
        ]
        self.bbox_shift = bbox_shift
        print(f"number of images: {len(self.gt_path_files)}")
    ...
    def __getitem__(self, index):
        ...
        # Check if the image size is (1024, 1024, 3), if not resize it
        if img_1024.shape != (1024, 1024, 3):
            img_1024 = np.array(Image.fromarray((img_1024 * 255).astype(np.uint8)).resize((1024, 1024)))
            img_1024 = img_1024.astype(np.float32) / 255.0

        # Check if image values are in the range [0, 1], if not normalize it
        if np.max(img_1024) > 1.0 or np.min(img_1024) < 0.0:
