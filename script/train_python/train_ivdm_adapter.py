...
import monai
from segment_anything_adapter import sam_model_registry
...
parser.add_argument(
    ...
    "--tr_npy_path",
    type=str,
    default="/mnt/sda/minkyukim/sam_dataset_refined/ivdm_npy_train_dataset_1024image",
...
model_save_path = "../../../../../../mnt/sda/minkyukim/pth/sam-tutorial_ivdm_adapter"
...
class MedSAM(nn.Module):
    def __init__(
        ...
        #     print(f"all parameter:{name}")
        for param in self.prompt_encoder.parameters():
            param.requires_grad = False

        for name, param in self.image_encoder.named_parameters():
            # Allow learning only for adapter parameters in image_encoder
            if "Adapter" in name:
                param.requires_grad = True
                # print(f"Trainable parameter in image_encoder: {name}")
            else:
                param.requires_grad = False
...
def main():
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        torch.save(sam_model.state_dict(), join(model_save_path, f"medsam_adapter_{epoch}.pth"))
        ## save the best model
        if epoch_loss < best_loss:
            best_loss = epoch_loss

            torch.save(sam_model.state_dict(), join(model_save_path, "medsam_adapter_best.pth"))
