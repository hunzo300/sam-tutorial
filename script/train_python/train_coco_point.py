...
model_save_path = "../../../../../../mnt/sda/minkyukim/pth/sam-tutorial_coco"
...
for step, (image, gt, bboxes, point_coords, point_labels, names_temp) in enumerate(tr_dataloader):
    ...
    plt.savefig("./data_sanitycheck_coco_point.png", bbox_inches="tight", dpi=300)
    plt.close()
...
def main():
    ...
    num_samples = len(train_dataset) // 8
    indices = np.random.choice(len(train_dataset), num_samples, replace=False)
    subset_train_dataset = Subset(train_dataset, indices)

    print("Number of samples used for training (1/8): ", len(subset_train_dataset))
    ...
    # Log or print losses here if necessary
        ...
        #     "epoch": epoch,
        # }
        torch.save(sam_model.state_dict(), join(model_save_path, "medsam_model_latest_point.pth"))
        ## save the best model
        if epoch_loss < best_loss:
            ...
            #     "epoch": epoch,
            # }
            torch.save(sam_model.state_dict(), join(model_save_path, "medsam_model_best_point.pth"))
