...
def unit_test(random_seed=42): 
    ...
    dataset = NpyDataset(f"/mnt/sda/minkyukim/sam_dataset/{dataset_}_npy_{test_train}_dataset_1024image")
    ...
    sam_model_2 = sam_model_registry[args.model_type](checkpoint=f"/mnt/sda/minkyukim/pth/sam-tutorial_{dataset_}/medsam_model_best_point.pth")
    ...
    for step, (image, gt, bboxes, point_coords, point_labels, names_temp) in enumerate(dataloader):
        ...
        plt.savefig(f"output_images/{dataset_}_point_{names_temp[-1]}_comparison.png", bbox_inches="tight", dpi=300)
        plt.close()
